...
- **Catálogo de telares**: el de `TELARES_DEFAULT` más los que use alguna
  ...
  (v9 lo mete en el catálogo). Ya no hay «Otro…» con prompt para escribir un
  telar nuevo: se guardaba nada más teclearlo y un valor puesto por error se
  quedaba para siempre (v8 limpió lo que había caído así). Un telar nuevo de
  verdad se añade a `TELARES_DEFAULT` y entra con la migración; por API sigue
  valiendo cualquier texto (`_anadir_a_catalogo` desde `crear`/`actualizar`).
...
- **Avisos por correo** (`shared/scripts/correo.py` + `muestras_avisos.py`): a
  ...
  **etapas de diseño** suman destinatarios (sale un único correo con todos):
  ...
  mandar nada— y el panel de Análisis avisa si no lo habla. El MTA de
  producción (Postfix 3.10 de Plesk) venía con `smtputf8_enable = no` y se
  activó a mano el 14/09/2026 (`postconf -e smtputf8_enable=yes && systemctl
  reload postfix`): si Plesk reescribiera `main.cf` habría que volver a
  ponerlo, y el panel lo cantaría en rojo. Si un día no lo hablara, el envío
  no se cae: `correo._sin_smtputf8()` deja esa dirección fuera, el correo sale
  para el resto y lo cuenta en el historial del aviso); «Listo para revisión
